...
.controller("AppCtrl", function AppCtrl($scope,$cordovaSQLite,$http,GeoLocation,DB) {
                ...
                success(function(data,status,headers,config) {
                    ...
                    angular.forEach(data, function (value,key) {
                        ...
                        for (var i =0;i<value.length; i++) {
                            ...
                            dates.push({"street":$scope.query.street,"hnr":$scope.query.hnr,"wtype":key,"col_date":date,"date_added":new Date()})
                        }
                    });
        ...
        function saveStreetChoice() {
            ...
            window.localStorage.setItem("street",$scope.query.street);
            window.localStorage.setItem("hnr",$scope.query.hnr);
        };
                ...
                if (res.rows.length > 0) {
                    ...
                    for(var i = 0 ; i<res.rows.length;i++) {
                        ...
                        if(loop.collection_date==loop_last_change_cd) {
                            ...
                            $scope.dates.push({"collection_date": loop.collection_date, "waste_type":[loop.waste_type]});
